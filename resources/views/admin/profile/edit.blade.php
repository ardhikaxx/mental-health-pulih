@extends('layouts.dashboard', ['title' => 'Profil Admin'])
...
<section class="hero-panel d-flex justify-content-between align-items-center mb-4">
    <div style="position: relative; z-index: 2;">
        ...
        <p class="mb-0">Perbarui data akun, email, dan informasi kontak Anda.</p>
    </div>
</section>
...
<div class="row g-4 mb-4">
    <div class="col-lg-8">
        <div class="card border-0 p-4 shadow-sm">
            ...
            <form method="POST" action="{{ route('admin.profile.update') }}">
                ...
                <div class="mt-4 pt-3 border-top text-end">
                    <button class="btn btn-primary px-4 shadow-sm" type="submit"><i class="fa-solid fa-floppy-disk me-2"></i> Simpan Profil</button>
                </div>
            </form>
        </div>
        ...
        <div class="card border-0 shadow-sm p-4 mt-4">
            <h5 class="fw-bold mb-4 pb-3 border-bottom"><i class="fa-solid fa-shield-halved text-primary me-2"></i> Keamanan Login</h5>
            <div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3">
                <div>
                    <p class="mb-1 fw-bold text-dark">Autentikasi 2 Langkah: {!! $user->two_factor_enabled ? '<span class="text-success">Aktif</span>' : '<span class="text-muted">Nonaktif</span>' !!}</p>
                    <p class="text-muted small mb-0">Wajibkan verifikasi kode email setiap kali Anda masuk ke dashboard admin.</p>
                </div>
                <form action="{{ route('admin.profile.two-factor') }}" method="POST">
                    @csrf
                    <button type="submit" class="btn {{ $user->two_factor_enabled ? 'btn-outline-danger' : 'btn-primary' }} px-4 fw-bold shadow-sm">
    ...
    <div class="col-lg-4">
        <div class="card border-0 p-4 shadow-sm bg-danger bg-opacity-10">
            ...
            <form method="POST" action="{{ route('admin.profile.destroy') }}" onsubmit="return confirm('Apakah Anda yakin ingin menghapus akun ini secara permanen?')">
                ...
                <button class="btn btn-danger w-100 shadow-sm fw-bold" type="submit"><i class="fa-solid fa-trash me-2"></i> Hapus Akun Permanen</button>
            </form>
        </div>
    </div>
</div>
